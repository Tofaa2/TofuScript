...
pub const Lexer = struct {
    ...
    fn identifier(self: *Lexer, token_list: *std.ArrayList(Token)) !void {
        ...
        const token_type = if (std.mem.eql(u8, text, "func")) TokenType.func else if (std.mem.eql(u8, text, "var")) TokenType.@"var" else if (std.mem.eql(u8, text, "if")) TokenType.@"if" else if (std.mem.eql(u8, text, "else")) TokenType.@"else" else if (std.mem.eql(u8, text, "return")) TokenType.@"return" else if (std.mem.eql(u8, text, "true")) TokenType.true else if (std.mem.eql(u8, text, "false")) TokenType.false else if (std.mem.eql(u8, text, "nil")) TokenType.nil else if (std.mem.eql(u8, text, "and")) TokenType.@"and" else if (std.mem.eql(u8, text, "or")) TokenType.@"or" else if (std.mem.eql(u8, text, "import")) TokenType.import else TokenType.identifier;
